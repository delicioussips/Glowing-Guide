...
// ─── CUPPING LOG ──────────────────────────────────────────────────────────────
const CUPP_EMPTY = {
  date:today(), cupper:"", sampleRefs:[], origin:"", bagMarks:"", roastDate:"", roastLevel:"",
  colorResult:"pass", tasteResult:"pass", smellResult:"pass",
  notes:""
};

function CuppingLog({ cuppingRecords, onSave, onUpdate, onDelete, orders }){
  ...
  const overallResult = (rec) => {
    return (rec.colorResult==="pass" && rec.tasteResult==="pass" && rec.smellResult==="pass") ? "PASS" : "FAIL";
  };

  const save = ()=>{
    const scored = {...d, result: overallResult(d), id:Date.now(), createdAt:new Date().toISOString()};
    onSave(scored);
  ...
  const saveEdit = ()=>{
    const scored = {...editData, result: overallResult(editData)};
    onUpdate(scored); setEditId(null);
    setFlash("edited"); setTimeout(()=>setFlash(""),2500);
  };

  const handleDelete = (rec) => {
    if(!window.confirm(`Delete cupping session for "${rec.origin||"Unknown"}"?`)) return;
    onDelete(rec.id);
    setFlash("deleted"); setTimeout(()=>setFlash(""),2500);
  };

  const resultColor = r => r==="PASS"||r==="pass" ? "#3DAA6A" : "#E8531A";
      ...
      <div style={{ display:"flex", gap:0, borderRadius:6, overflow:"hidden", border:"1px solid rgba(255,255,255,0.15)" }}>
        {["pass","fail"].map(opt=>(
          <button key={opt} onClick={()=>onChange(opt)} style={{
            flex:1, padding:"8px 14px", border:"none", cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em",
  ...
  const SampleRefPicker = ({ values, onChange }) => {
    const addRef = (val) => { if(val && !values.includes(val)) onChange([...values, val]); };
    const removeRef = (val) => onChange(values.filter(v=>v!==val));
    return (
      <div>
        <label style={lbl}>Sample References (multiple)</label>
        <select style={inp()} value="" onChange={e=>{addRef(e.target.value); e.target.value="";}}>
          <option value="">— add PO / Batch —</option>
          {(orders||[]).flatMap(po=>[
        ...
        {values.length>0 && (
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:8 }}>
            {values.map(v=>(
              <div key={v} style={{ display:"flex", alignItems:"center", gap:5, background:"rgba(168,85,247,0.15)", border:"1px solid rgba(168,85,247,0.3)", borderRadius:5, padding:"3px 8px", fontSize:11, color:"#A855F7" }}>
                {v}
                <button onClick={()=>removeRef(v)} style={{ background:"none", border:"none", color:"#E8531A", cursor:"pointer", fontSize:13, fontFamily:"inherit", padding:0, lineHeight:1 }}>×</button>
              </div>
            ))}
          </div>
        )}
  ...
  const renderForm = (rec, setRec, onSubmit, submitLabel)=>(
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(175px,1fr))", gap:"12px 18px", marginBottom:20 }}>
        ...
        <div style={{ gridColumn:"1/-1" }}>
          <SampleRefPicker values={rec.sampleRefs||[]} onChange={v=>setRec("sampleRefs",v)}/>
        </div>
      ...
      <div style={{ background:"rgba(91,164,164,0.07)", border:"1px solid rgba(91,164,164,0.2)", borderRadius:10, padding:"16px 18px", marginBottom:18 }}>
        <div style={{ fontSize:12, fontWeight:700, color:"#5BA4A4", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.08em" }}>Quality Assessment — Pass / Fail</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:"14px 22px" }}>
          <PassFailToggle label="Color" value={rec.colorResult||"pass"} onChange={v=>setRec("colorResult",v)}/>
          <PassFailToggle label="Taste" value={rec.tasteResult||"pass"} onChange={v=>setRec("tasteResult",v)}/>
          <PassFailToggle label="Smell" value={rec.smellResult||"pass"} onChange={v=>setRec("smellResult",v)}/>
        </div>
        <div style={{ marginTop:14, padding:"10px 14px", background:"rgba(91,164,164,0.1)", borderRadius:8, display:"flex", alignItems:"center", gap:14 }}>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.5)" }}>Overall Result:</div>
          <div style={{ fontSize:20, fontWeight:700, color:resultColor(overallResult(rec)), letterSpacing:"0.06em" }}>{overallResult(rec)}</div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)" }}>(all criteria must pass)</div>
        </div>
      </div>

      <div style={{ marginBottom:18 }}>
        <label style={lbl}>General Notes / Tasting Notes</label>
        <Fld type="textarea" value={rec.notes} onChange={v=>setRec("notes",v)} placeholder="Flavor profile, aroma characteristics, body, acidity observations…"/>
      </div>
              ...
              <div key={rec.id} style={{ border:`1px solid rgba(91,164,164,0.25)`, borderRadius:10, overflow:"hidden" }}>
                ...
                <div style={{ background:"rgba(91,164,164,0.08)", padding:"13px 18px", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                  <div style={{ flex:1 }}>
                    ...
                    {(rec.sampleRefs||[]).length>0 && (
                      <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginTop:5 }}>
                        {rec.sampleRefs.map(s=><span key={s} style={{ fontSize:10, color:"#A855F7", background:"rgba(168,85,247,0.1)", border:"1px solid rgba(168,85,247,0.25)", borderRadius:4, padding:"1px 6px" }}>{s}</span>)}
                      </div>
                    )}
                    {/* Legacy single sampleRef */}
                    {!rec.sampleRefs?.length && rec.sampleRef && <div style={{ fontSize:11, color:"#A855F7", marginTop:3 }}>{rec.sampleRef}</div>}
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    {!isEditing && <button onClick={()=>{ setEditId(rec.id); setEditData({...rec, sampleRefs: rec.sampleRefs||[]}); }} style={ghost({padding:"5px 13px",fontSize:11,color:"#5BA4A4",borderColor:"#5BA4A455"})}>Edit</button>}
                ...
                {!isEditing && (
                  <div style={{ padding:"12px 18px 14px" }}>
                    <div style={{ display:"flex", gap:16, flexWrap:"wrap", marginBottom:8 }}>
                      {[["Color",rec.colorResult],["Taste",rec.tasteResult],["Smell",rec.smellResult]].map(([label,val])=>(
                        <div key={label} style={{ display:"flex", alignItems:"center", gap:6 }}>
                          <span style={{ fontSize:11, color:"rgba(255,255,255,0.45)" }}>{label}:</span>
                          <span style={{ fontSize:12, fontWeight:700, color:resultColor(val||"pass"), textTransform:"uppercase" }}>{val||"pass"}</span>
                        </div>
                      ))}
                    </div>
                    {rec.notes && <div style={{ marginTop:6, fontSize:12, color:"rgba(255,255,255,0.45)", fontStyle:"italic", padding:"8px 12px", background:"rgba(255,255,255,0.03)", borderRadius:6 }}>📝 {rec.notes}</div>}
                  </div>
                )}
    ...
    <div style={{ minHeight:"100vh", background:"#0E0A07", fontFamily:"'Georgia','Times New Roman',serif", color:"#F5EDD8", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, padding:"20px 22px", maxWidth:1200, width:"100%", boxSizing:"border-box" }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.065)", borderRadius:12, padding:20 }}>
          ...
          {active==="cupping"    && <CuppingLog cuppingRecords={cuppingRecords} onSave={handleSaveCupping} onUpdate={handleUpdateCupping} onDelete={handleDeleteCupping} orders={orders}/>}
