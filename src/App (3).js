...
function parseOrderText(text) {
  const ROAST_LEVELS = {
    'light':'Light','blonde':'Light',
    'medium-light':'Medium-Light','medium light':'Medium-Light',
    'medium':'Medium','med':'Medium',
    'medium-dark':'Medium-Dark','medium dark':'Medium-Dark',
    'dark':'Dark','espresso':'Dark','french':'Dark','italian':'Dark',
  };
  const COFFEE_ORIGINS = new Set([
    'ethiopia','ethiopian','colombia','colombian','brazil','brazilian',
    'guatemala','guatemalan','kenya','kenyan','costa rica','honduras',
    'honduran','peru','peruvian','mexico','mexican','sumatra','sumatran',
    'java','rwanda','rwandan','burundi','tanzania','tanzanian','uganda',
    'ugandan','papua','nicaragua','nicaraguan','el salvador','salvadoran',
    'india','indian','yemen','yemeni','congo','congolese','bolivia',
    'bolivian','panama','panamanian','hawaiian','hawaii','kona',
    'geisha','geshe','sidamo','yirgacheffe','huehuetenango','antigua',
    'tarrazu','mandheling','sulawesi',
  ]);
  const SIZES = {
    '8oz':'8oz','8 oz':'8oz','12oz':'12oz','12 oz':'12oz',
    '1lb':'1lb','1 lb':'1lb','1 pound':'1lb','2lb':'2lb','2 lb':'2lb',
    '5lb':'5lb','5 lb':'5lb','10lb':'10lb','bulk':'Bulk',
  };
  const STOP_WORDS = new Set(['and','also','plus','with','by','due','before',
    'need','needed','deliver','delivery','ship','thanks','thank','regards',
    'cheers','best','sincerely','hi','hey','hello','can','you','do','please',
    'we','i','the','a','an','for','order','want','like','send','get','put',
    'make','of','from','subject','to','cc','re','fw','fwd','some','me','us']);

  function isCoffeeTerm(w) {
    const wl = w.toLowerCase().replace(/[^a-z -]/g,'');
    if (ROAST_LEVELS[wl]) return true;
    if (COFFEE_ORIGINS.has(wl)) return true;
    if (SIZES[wl]) return true;
    if (/^(?:roast|blend|bean|ground|whole|decaf|single.origin|house|breakfast|signature|custom|seasonal)$/i.test(wl)) return true;
    return false;
  }
  ...
  if (fromMatch) customer = (fromMatch[1] || fromMatch[2] || '').replace(/@.*/, '').replace(/<.*/, '').trim();
  const subjectMatch = text.match(/(?:^|\n)\s*Subject:\s*(.+)/i);
  if (subjectMatch) notes = subjectMatch[1].trim();

  // ── Extract from signature ──
  if (!customer) {
    const sigMatch = text.match(/(?:thanks|regards|cheers|best|sincerely)[,!]?\s*[-\u2013\u2014]?\s*([A-Z][a-z]+(?: [A-Z][a-z]+){0,2})/i);
    if (sigMatch) customer = sigMatch[1].trim();
  }

  // ── Priority ──
  if (/\b(rush|asap|emergency)\b/i.test(full)) priority = 'Rush';
  else if (/\b(urgent|soon|quick|priority)\b/i.test(full)) priority = 'Urgent';

  // ── Due date ──
  const dateMatch = full.match(/(?:by|due|before|deliver|need(?:ed)?)\s+([A-Za-z]+day|\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|\w+ \d{1,2}(?:st|nd|rd|th)?(?:,? \d{4})?)/i);
  if (dateMatch) {
    ...
    if (!isNaN(d.getTime()) && d.getFullYear() > 2020) targetDate = d.toISOString().slice(0,10);
      ...
      if (dayIdx >= 0) { const now = new Date(); now.setDate(now.getDate() + ((dayIdx - now.getDay() + 7) % 7 || 7)); targetDate = now.toISOString().slice(0,10); }
    }
  }

  // ── Strip email scaffolding (whole words only to preserve origins like Ethiopian) ──
  let bodyText = full;
  bodyText = bodyText.replace(/\b(?:From|To|Cc|Subject|Date):\s*\S+(?:\s+\S+)*/gi, ' ');
  bodyText = bodyText.replace(/\b(?:hi|hey|hello|good morning|good afternoon)\b[,!]?\s*/gi, ' ');
  bodyText = bodyText.replace(/\b(?:can you|could you|please|we need|i need|we want|i want|send us|put together|we'd like)\b/gi, ' ');
  bodyText = bodyText.replace(/\b(?:thanks|thank you|regards|cheers|best|sincerely)\b[\s\S]*/gi, ' ');
  bodyText = bodyText.replace(/\b(?:need(?:ed)?\s+by|due\s+by|deliver(?:y)?\s+by|by)\b\s+\S+/gi, ' ');
  bodyText = bodyText.replace(/[.!?,;:()]+/g, ' ');
  bodyText = bodyText.replace(/\b(?:and|also|plus)\b/gi, ' ');
  bodyText = bodyText.replace(/\s+/g, ' ').trim();

  const tokens = bodyText.split(/\s+/).filter(Boolean);
  let ti = 0;

  // ── Leading non-number non-coffee words = customer name ──
  if (!customer) {
    const cw = [];
    while (ti < tokens.length) {
      const t = tokens[ti];
      if (/^\d+$/.test(t)) break;
      if (isCoffeeTerm(t)) break;
      if (STOP_WORDS.has(t.toLowerCase().replace(/[^a-z]/g,'')) && cw.length > 0) break;
      cw.push(t);
      ti++;
    }
    customer = cw.join(' ').replace(/[-\u2013\u2014:,]+$/, '').trim();
  }

  // ── Parse qty + product pairs ──
  while (ti < tokens.length) {
    const t = tokens[ti];
    if (/^\d+$/.test(t)) {
      const qty = t;
      ...
      while (ti < tokens.length && !/^\d+$/.test(tokens[ti])) {
        const w = tokens[ti];
        const wl = w.toLowerCase().replace(/[^a-z0-9 -]/g, '');

        // Check two-word combos first (medium dark, house blend, 1 lb, etc)
        if (ti + 1 < tokens.length) {
          const next = tokens[ti+1].toLowerCase().replace(/[^a-z0-9 -]/g, '');
          const combo = wl + ' ' + next;
          if (ROAST_LEVELS[combo] && !roast) { roast = ROAST_LEVELS[combo]; ti += 2; continue; }
          if (SIZES[combo] && !size) { size = SIZES[combo]; ti += 2; continue; }
        }
        // Single-word checks
        if (ROAST_LEVELS[wl] && !roast) { roast = ROAST_LEVELS[wl]; }
        else if (SIZES[wl] && !size) { size = SIZES[wl]; }
        else if (!STOP_WORDS.has(wl)) { nameWords.push(w); }
        ti++;
      }

      // Build product name
      let productName = nameWords.join(' ').replace(/\b\w/g, c => c.toUpperCase()).trim();
      // If name is empty or just "Roast"/"Blend", prepend roast profile
      if (!productName && roast) productName = roast + ' Roast';
      else if (/^(?:roast|bean|ground)$/i.test(productName) && roast) productName = roast + ' ' + productName;
      if (!productName) continue;

      const nl = productName.toLowerCase();
      products.push({
        productName, roastProfile: roast, packageSize: size,
        skuQty: qty, totalLbs: '',
        requiresRoast: true,
        requiresGrinding: /ground/i.test(nl),
        requiresBlending: /blend/i.test(nl),
        requiresPackaging: true, notes: ''
      });
    } else { ti++; }
  }
...
function parseDOText(text) {
  ...
  const p1 = new RegExp("(\\d+)\\s*(?:bags?|sacks?)\\s+(?:of\\s+)?((?:"+COFFEE_ORIGINS+")[\\w\\s/-]*?)\\s+(?:lot|mark|ref)?[#:\\s-]*(\\S+)\\s*(?:@\\s*)?(\\d+(?:\\.\\d+)?)\\s*(?:lbs?|kg|pounds?)", "gi");
  let m;
  ...
  const p2 = new RegExp("((?:"+COFFEE_ORIGINS+")[\\w\\s]*?)[/,|]+\\s*(?:lot|mark)?[#:\\s-]*(\\S+)[/,|]+\\s*(\\d+)\\s*bags?[/,|]+\\s*(\\d+(?:\\.\\d+)?)\\s*(?:lbs?|kg)", "gi");
  ...
  // Pattern 3: simple "qty origin" pairs (like production orders but for receiving)
  if (items.length === 0) {
    const originRe = new RegExp("(\\d+)\\s*(?:bags?\\s+(?:of\\s+)?)?((?:"+COFFEE_ORIGINS+")(?:\\s+[A-Za-z]+)*)", "gi");
    while ((m = originRe.exec(full)) !== null) {
      ...
      items.push({ date, supplier, deliveryOrder, origin: origin.replace(/\b\w/g, c => c.toUpperCase()),
        bagMarks: lotM ? lotM[1] : "", qtyBags: m[1], weightPerBag: wtM ? wtM[1] : "",
        costPerLb: "", coaFile: null, notes: "", id: Date.now() + items.length });
    }
  }

  // Fallback: find any origin mentions without qty
  if (items.length === 0) {
    const looseRe = new RegExp("((?:"+COFFEE_ORIGINS+")(?:\\s+[A-Za-z]+)*)", "gi");
    while ((m = looseRe.exec(full)) !== null) {
      const origin = m[1].trim();
      if (origin.length < 3 || seenOrigins.has(origin.toLowerCase())) continue;
      seenOrigins.add(origin.toLowerCase());
      const ctx = full.slice(Math.max(0, m.index - 80), m.index + m[0].length + 80);
      ...
      const wtM = ctx.match(/(\d+(?:\.\d+)?)\s*(?:lbs?|kg|pounds?)/i);
      items.push({ date, supplier, deliveryOrder, origin: origin.replace(/\b\w/g, c => c.toUpperCase()),
        bagMarks: "", qtyBags: qtyM ? qtyM[1] : "", weightPerBag: wtM ? wtM[1] : "",
        costPerLb: "", coaFile: null, notes: "", id: Date.now() + items.length });
    }
  }
...
function ReceivingLog({ onSave, records, onUpdateRecord }){
  ...
  const handleDOUpload = async (file)=>{
    if(!file) return;
    if(!ANTHROPIC_KEY){
      setDoError("No API key configured — file upload parsing requires an Anthropic API key. Use the \"Paste Text\" tab instead to parse delivery orders instantly.");
      setDoFile(null);
      return;
    }
...
- Return ONLY the JSON array, starting with [ and ending with ]`;
      ...
      console.log("[DO] calling Anthropic model=claude-3-5-haiku-20241022 fileType=" + file.type);
...
- Return ONLY the JSON object, nothing else`;
  ...
  const parseLocal = () => {
    ...
    try {
      ...
      // Accept if we got a customer OR any product with a name — user can edit the rest
      const hasAnyProduct = result.products.some(p => p.productName);
      if (!result.customer && !hasAnyProduct) {
        setError("Could not extract order details. Try format like: CustomerName 2 Brazil 4 dark 3 medium — or paste an email with product quantities.");
        return;
      }
      setParsed(result);
    } catch(e) {
  ...
  const parseWithAI = async () => {
    ...
    if(!ANTHROPIC_KEY) return setError("No API key configured. Use '⚡ Quick Parse' instead, or set REACT_APP_ANTHROPIC_KEY in your Netlify environment variables and redeploy.");
    ...
    try {
      ...
      console.log("[PO] calling Anthropic model=claude-3-5-haiku-20241022 hasFile=" + hasFile + " hasText=" + hasText);
      const poResp = await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",
        headers: poHeaders,
        body:JSON.stringify({ model:"claude-3-5-haiku-20241022", max_tokens:2000, system:AI_SYSTEM, messages:[{ role:"user", content:messageContent }]})
      });
        ...
        <div style={{ padding:"20px 22px" }}>
          ...
          {confirmed ? (
            ...
            <>
              ...
              {!ANTHROPIC_KEY && <div style={{ marginTop:10, fontSize:11, color:"rgba(255,255,255,0.3)", fontStyle:"italic" }}>💡 Quick Parse works instantly on pasted text. To parse images/PDFs with AI, add your API key to Netlify env vars.</div>}
            </>
