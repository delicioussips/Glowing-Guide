...
const deleteReceiving = async (id) => {
  const { error } = await supabase.from('receiving').delete().eq('id', id);
  if (error) throw error;
};
const deleteCupping = async (id) => {
  const { error } = await supabase.from('cupping').delete().eq('id', id);
  if (error) throw error;
};
const deleteAdjustment = async (id) => {
  const { error } = await supabase.from('inv_adjustments').delete().eq('id', id);
  if (error) throw error;
};
...
const fetchCustomers = async () => {
  ...
  if (error) {
    ...
    console.warn("customers table not found, using fallback");
    return [];
  }
  return (data || []).map(r => r.data?.name || r.data || "").filter(Boolean);
};
const insertCustomer = async (name) => {
  const { error } = await supabase
    .from('customers')
    .insert([{ data: { name, id: Date.now() } }]);
  if (error) console.warn("Could not save customer:", error.message);
};

// ─── REAL-TIME SUBSCRIPTIONS ─────────────────────────────────────────────────
// Call this once on app load — fires callback whenever any employee changes data
const subscribeAll = (onOrdersChange, onReceivingChange, onAdjChange, onCuppingChange, onCustomersChange) => {
  const channel = supabase
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'customers' },     () => onCustomersChange?.())
    .subscribe();
...
// ─── LOCAL TEXT PARSERS (no API key required) ─────────────────────────────

// Parse a pasted email/SMS/text into a Production Order
function parseOrderText(text) {
  ...
  if (fromMatch) customer = (fromMatch[1] || fromMatch[2] || "").replace(/@.*/, "").trim();
  if (!customer) {
    const sigMatch = text.match(/(?:thanks|regards|cheers|best|sincerely)[,!]?\s*[-–—]?\s*([A-Z][a-z]+(?: [A-Z][a-z]+)*)/i);
    if (sigMatch) customer = sigMatch[1].trim();
  }
  if (!customer) {
    const dashSig = text.match(/[-–—]\s*([A-Z][a-z]+(?: [A-Z][a-z]+){0,2})\s*$/m);
    if (dashSig) customer = dashSig[1].trim();
  }

  // ── Dates ──
  let targetDate = "";
  const dueMatch = full.match(/(?:by|due|before|deliver(?:y|ed)?(?:\s+by)?|need(?:ed)?\s+by|ship\s+by)[:\s]+([A-Za-z0-9,/\- ]+?)(?:\.|$|\s{2})/i);
  if (dueMatch) {
    const raw = dueMatch[1].trim();
    const d = new Date(raw);
    if (!isNaN(d.getTime()) && d.getFullYear() > 2020) { targetDate = d.toISOString().slice(0,10); }
    else {
      const days = ["sunday","monday","tuesday","wednesday","thursday","friday","saturday"];
      const dayIdx = days.findIndex(dy => raw.toLowerCase().includes(dy));
      if (dayIdx >= 0) {
        const now = new Date();
  ...
  if (!targetDate) {
    const genDate = full.match(/(\d{4}[-/]\d{1,2}[-/]\d{1,2})|(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})/);
    if (genDate) { const d = new Date(genDate[0]); if (!isNaN(d.getTime())) targetDate = d.toISOString().slice(0,10); }
  }

  // ── Priority ──
  let priority = "Normal";
  if (/\b(rush|asap|emergency|urgent!)\b/i.test(full)) priority = "Rush";
  else if (/\b(urgent|soon|quick|priority)\b/i.test(full)) priority = "Urgent";

  // ── Products ──
  const products = [];
  const seen = new Set();
  const productPatterns = [
    /(\d+)\s*(?:units?|bags?|cases?|boxes?|ct|x|×)?\s*(?:of\s+)?([A-Za-z][\w\s/-]*?)\s+(8\s*oz|12\s*oz|1\s*lb|2\s*lb|5\s*lb|bulk)\b/gi,
    /([A-Za-z][\w\s/-]*?)\s+(8\s*oz|12\s*oz|1\s*lb|2\s*lb|5\s*lb|bulk)\s*[x×—-]\s*(\d+)/gi,
    /(\d+)\s+(?:units?\s+(?:of\s+)?)?([A-Za-z][\w\s/-]*?(?:roast|blend|espresso|coffee|bean|ground|whole)[\w\s/-]*?)(?=\s*[.,;]|\s+and\b|\s+also\b|\s*$)/gi,
  ];

  for (let pi = 0; pi < productPatterns.length; pi++) {
    const pat = productPatterns[pi];
    let m;
    while ((m = pat.exec(full)) !== null) {
      let qty, name, size;
      if (pi === 0) { qty = m[1]; name = m[2]; size = m[3]; }
      else if (pi === 1) { name = m[1]; size = m[2]; qty = m[3]; }
      else { qty = m[1]; name = m[2]; size = ""; }
      name = name.replace(/^\s+|\s+$/g, "").replace(/\s{2,}/g, " ");
      if (name.length < 3 || seen.has(name.toLowerCase())) continue;
      seen.add(name.toLowerCase());
      let roast = "";
      const nl = name.toLowerCase();
      if (/\bmedium[\s-]*dark\b/i.test(nl)) roast = "Medium-Dark";
      else if (/\bmedium[\s-]*light\b/i.test(nl)) roast = "Medium-Light";
      else if (/\bdark\b|\bespresso\b/i.test(nl)) roast = "Dark";
      else if (/\bmedium\b/i.test(nl)) roast = "Medium";
      else if (/\blight\b|\bblonde\b/i.test(nl)) roast = "Light";
      size = (size || "").replace(/\s+/g, "").toLowerCase();
      const sizeMap = {"8oz":"8oz","12oz":"12oz","1lb":"1lb","2lb":"2lb","5lb":"5lb","bulk":"Bulk"};
      size = sizeMap[size] || "";
      products.push({
        productName: name.replace(/\b\w/g, c => c.toUpperCase()),
        roastProfile: roast, packageSize: size, skuQty: qty || "", totalLbs: "",
        requiresRoast: true, requiresGrinding: /\bground\b/i.test(nl),
        requiresBlending: /\bblend\b/i.test(nl), requiresPackaging: true, notes: ""
      });
    }
  }
  if (products.length === 0) {
    products.push({ productName: "", roastProfile: "", packageSize: "", skuQty: "", totalLbs: "",
      requiresRoast: true, requiresGrinding: false, requiresBlending: false, requiresPackaging: true, notes: "" });
  }

  let notes = "";
  const subjectMatch = text.match(/(?:^|\n)\s*Subject:\s*(.+)/i);
  if (subjectMatch) notes = subjectMatch[1].trim();

  return { customer, date: today(), targetDate, priority, notes, products };
}

// Parse delivery order text into receiving line items
function parseDOText(text) {
  const full = text.replace(/\r?\n/g, " ");

  let supplier = "";
  const suppMatch = text.match(/(?:from|supplier|shipper|vendor|company)[:\s]+([^\n,]+)/i);
  if (suppMatch) supplier = suppMatch[1].trim();

  let deliveryOrder = "";
  const doMatch = full.match(/(?:D\.?O\.?|P\.?O\.?|delivery\s*order|purchase\s*order|order|invoice|ref)[\s#:]*([A-Z0-9][-A-Z0-9]+)/i);
  if (doMatch) deliveryOrder = doMatch[1].trim();

  let date = today();
  const dateMatch = full.match(/(\d{4}[-/]\d{1,2}[-/]\d{1,2})|(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})/);
  if (dateMatch) { const d = new Date(dateMatch[0]); if (!isNaN(d.getTime())) date = d.toISOString().slice(0, 10); }

  const items = [];
  const origins = "ethiopia|ethiopian|colombi|brazil|brazilian|guatemala|guatemalan|kenya|kenyan|costa rica|honduras|honduran|peru|peruvian|mexico|mexican|sumatra|sumatran|java|rwanda|rwandan|burundi|tanzania|tanzanian|uganda|ugandan|papua|nicaragua|nicaraguan|el salvador|india|indian|yemen|congo|bolivia|bolivian|panama|panamanian|hawaiian?|kona|geisha|geshe|sidamo|yirgacheffe|huehuetenango|antigua|tarrazu|mandheling|sulawesi";
  const seenOrigins = new Set();

  // Pattern 1: "10 bags Ethiopia Yirgacheffe LOT-123 152lbs"
  const p1 = new RegExp("(\\d+)\\s*(?:bags?|sacks?)\\s+(?:of\\s+)?((?:"+origins+")[\\w\\s/-]*?)\\s+(?:lot|mark|ref)?[#:\\s-]*(\\S+)\\s*(?:@\\s*)?(\\d+(?:\\.\\d+)?)\\s*(?:lbs?|kg|pounds?)", "gi");
  let m;
  while ((m = p1.exec(full)) !== null) {
    const origin = m[2].trim();
    if (seenOrigins.has(origin.toLowerCase())) continue;
    seenOrigins.add(origin.toLowerCase());
    items.push({ date, supplier, deliveryOrder, origin: origin.replace(/\b\w/g, c => c.toUpperCase()),
      bagMarks: m[3].trim(), qtyBags: m[1], weightPerBag: m[4], costPerLb: "", coaFile: null, notes: "", id: Date.now()+items.length });
  }

  // Pattern 2: "Ethiopia / LOT-123 / 10 bags / 152 lbs"
  const p2 = new RegExp("((?:"+origins+")[\\w\\s]*?)[/,|]+\\s*(?:lot|mark)?[#:\\s-]*(\\S+)[/,|]+\\s*(\\d+)\\s*bags?[/,|]+\\s*(\\d+(?:\\.\\d+)?)\\s*(?:lbs?|kg)", "gi");
  while ((m = p2.exec(full)) !== null) {
    const origin = m[1].trim();
    if (seenOrigins.has(origin.toLowerCase())) continue;
    seenOrigins.add(origin.toLowerCase());
    items.push({ date, supplier, deliveryOrder, origin: origin.replace(/\b\w/g, c => c.toUpperCase()),
      bagMarks: m[2].trim(), qtyBags: m[3], weightPerBag: m[4], costPerLb: "", coaFile: null, notes: "", id: Date.now()+items.length });
  }

  // Fallback: find any origin mentions
  if (items.length === 0) {
    const originRe = new RegExp("((?:"+origins+")(?:[\\s/,.-]+[\\w\\s/-]*?)?)", "gi");
    while ((m = originRe.exec(full)) !== null) {
      const origin = m[1].trim().split(/[/,|]/).shift().trim();
      if (origin.length < 3 || seenOrigins.has(origin.toLowerCase())) continue;
      seenOrigins.add(origin.toLowerCase());
      const ctx = full.slice(Math.max(0, m.index - 80), m.index + m[0].length + 80);
      const qtyM = ctx.match(/(\d+)\s*(?:bags?|sacks?)/i);
      const wtM = ctx.match(/(\d+(?:\.\d+)?)\s*(?:lbs?|kg|pounds?)/i);
      const lotM = ctx.match(/(?:lot|mark|ref)[#:\s-]*(\S+)/i);
      items.push({ date, supplier, deliveryOrder, origin: origin.replace(/\b\w/g, c => c.toUpperCase()),
        bagMarks: lotM ? lotM[1] : "", qtyBags: qtyM ? qtyM[1] : "", weightPerBag: wtM ? wtM[1] : "",
        costPerLb: "", coaFile: null, notes: "", id: Date.now()+items.length });
    }
  }

  if (items.length === 0) {
    items.push({ date, supplier, deliveryOrder, origin: "", bagMarks: "", qtyBags: "", weightPerBag: "",
      costPerLb: "", coaFile: null, notes: "", id: Date.now() });
  }
  return items;
}
...
function printReport(po){
  ...
  const productsHtml = po.products.map((prod,i)=>{
    ...
    const stepsHtml = reqSteps.map(step=>{
      ...
      const fields = {
        roast:     ["roastDate","totalGreenWt","roastedWeight","startTemp","endTemp","duration","colorScore","operator"],
        grinding:  ["grindDate","inputWeight","outputWeight","grindSize","colorScore","operator"],
...
function ReceivingLog({ onSave, records, onUpdateRecord, onDeleteRecord }){
  ...
  const [doInputMode, setDoInputMode] = useState("drop"); // "drop" | "paste"
  const [doText, setDoText] = useState("");  // pasted DO text
...
- Return ONLY the JSON array, starting with [ and ending with ]`;
      ...
      if(isPDF) doHeaders["anthropic-beta"] = "pdfs-2024-09-25";

      console.log("[DO] calling Anthropic model=claude-haiku-4-5-20251001 fileType=" + file.type);
  ...
  // Local text-based DO parsing (no API key needed)
  const handleDOTextParse = () => {
    if (!doText.trim()) return setDoError("Please paste delivery order text first.");
    setDoError("");
    try {
      const items = parseDOText(doText);
      setParsedItems(items);
      setFlash("do_parsed");
      setTimeout(()=>setFlash(""), 8000);
      if (items.length === 1) {
      ...
      setDoError("Could not parse text — please check format and try again.");
    }
  };
  ...
  return (
    <div>
      {/* DO Upload / Paste zone */}
      <div style={{ marginBottom:22 }}>
        <div style={{ fontSize:13, fontWeight:700, color:"rgba(255,255,255,0.7)", marginBottom:10 }}>
          📄 Import Delivery Order — Auto-fill from document or text
        </div>
        {/* Tab toggle */}
        <div style={{ display:"flex", gap:0, marginBottom:12, borderRadius:8, overflow:"hidden", border:"1px solid rgba(200,112,42,0.3)" }}>
          {[["drop","📎 Drop / Upload File"],["paste","✏️ Paste Text"]].map(([m,label])=>(
            <button key={m} onClick={()=>setDoInputMode(m)} style={{ flex:1, padding:"9px 0", border:"none", background:doInputMode===m?"rgba(200,112,42,0.18)":"rgba(255,255,255,0.03)", color:doInputMode===m?"#C8702A":"rgba(255,255,255,0.38)", cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:doInputMode===m?700:400, borderRight:m==="drop"?"1px solid rgba(200,112,42,0.2)":"none" }}>{label}</button>
          ))}
        </div>
          ...
          <>
            <div onClick={()=>doRef.current.click()}
              ...
              {uploading
                ...
                : <><span style={{ fontSize:22 }}>📂</span><div><div style={{ color:"rgba(255,255,255,0.6)", fontWeight:600 }}>Click or drag & drop Delivery Order</div><div style={{ fontSize:11, color:"rgba(255,255,255,0.35)", marginTop:3 }}>PDF, JPG, or PNG — {ANTHROPIC_KEY ? "AI will extract fields" : "upload supported when API key is configured"}</div></div></>
              }
            </div>
            <input ref={doRef} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display:"none" }} onChange={e=>handleDOUpload(e.target.files[0])}/>
            {!ANTHROPIC_KEY && <div style={{ marginTop:8, fontSize:11, color:"rgba(255,255,255,0.3)", fontStyle:"italic" }}>💡 No API key detected — use the "Paste Text" tab to parse delivery orders instantly without AI.</div>}
          </>
        )}

        {doInputMode==="paste" && (
          <div>
            <textarea rows={7} style={{ width:"100%", background:"rgba(255,255,255,0.06)", border:"1px solid rgba(200,112,42,0.3)", borderRadius:8, color:"#F5EDD8", padding:"12px 14px", fontSize:12, lineHeight:1.55, fontFamily:"monospace", outline:"none", resize:"vertical", boxSizing:"border-box" }}
              value={doText} onChange={e=>setDoText(e.target.value)}
              placeholder={"Paste your delivery order, email, or invoice text here…\n\nExample:\nDelivery Order #DO-2025-0847\nSupplier: Green Bean Imports\n10 bags Ethiopia Yirgacheffe LOT-4421 152lbs\n5 bags Colombia Huila LOT-3390 152lbs\n8 bags Guatemala Antigua LOT-5512 132lbs"}/>
            <button onClick={handleDOTextParse} style={{ marginTop:10, background:"#C8702A", color:"#fff", border:"none", borderRadius:6, padding:"9px 22px", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit", letterSpacing:"0.04em", display:"flex", alignItems:"center", gap:8 }}>
              ⚡ Parse & Extract Items
            </button>
          </div>
        )}
      ...
      {parsedItems.length>0 && (
        <div style={{ marginBottom:22, background:"rgba(61,170,106,0.06)", border:"1px solid rgba(61,170,106,0.2)", borderRadius:10, padding:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(165px,1fr))", gap:"10px 14px" }}>
                  ...
                  <div style={{ gridColumn:"1/-1" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <input type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" style={{ fontSize:11, color:"rgba(255,255,255,0.5)" }}
                        onChange={e=>setParsedItems(ps=>ps.map(p=>p.id===item.id?{...p,coaFile:e.target.files[0]||null}:p))}/>
                      {item.coaFile && <span style={{ color:"#3DAA6A", fontSize:11 }}>✓ {item.coaFile.name}</span>}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <button
            onClick={()=>{
              parsedItems.forEach((item,idx)=>{ if(item.origin||item.bagMarks) onSave({...item, id:Date.now()+idx*7}); });
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(195px,1fr))", gap:"14px 20px", marginBottom:22 }}>
        ...
        <div style={{ gridColumn:"1/-1" }}>
          <label style={lbl}>Certificate of Analysis (COA)</label>
          <div onClick={()=>coaRef.current.click()} style={inp({ cursor:"pointer", display:"flex", alignItems:"center", gap:10, color:d.coaFile?"#3DAA6A":"rgba(255,255,255,0.28)", borderStyle:"dashed", borderColor:d.coaFile?"#3DAA6A66":"rgba(255,255,255,0.13)" })}>
            <span style={{ fontSize:18 }}>📎</span>
            <span style={{ fontSize:13 }}>{d.coaFile?`✓ ${d.coaFile.name}`:"Click to attach COA (PDF, JPG, PNG, DOCX…)"}</span>
          </div>
          <input ref={coaRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" style={{ display:"none" }} onChange={e=>sd("coaFile",e.target.files[0]||null)}/>
        </div>
      </div>
                ...
                {records.map(r=>(
                  editId===r.id
                    ...
                    : <tr key={r.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                        ...
                        <td style={{ padding:"8px 10px" }}>{r.coaFile?<a href={typeof r.coaFile==="string"?r.coaFile:URL.createObjectURL(r.coaFile)} target="_blank" rel="noopener noreferrer" style={{ color:"#3DAA6A",fontSize:11,textDecoration:"underline",cursor:"pointer" }}>📎 {r.coaFile.name||"View COA"}</a>:<span style={{ color:"rgba(255,255,255,0.18)" }}>—</span>}</td>
                        <td style={{ padding:"8px 10px", display:"flex", gap:6 }}>
                          <button onClick={()=>startEdit(r)} style={ghost({padding:"3px 10px",fontSize:11,color:"#C8702A",borderColor:"#C8702A55"})}>Edit</button>
                          <button onClick={()=>{if(window.confirm("Delete this receiving entry?")) onDeleteRecord(r.id);}} style={ghost({padding:"3px 10px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>Delete</button>
                        </td>
                      </tr>
                ))}
              </tbody>
...
function InventoryLog({ receivingRecords, orderPulls, invAdjustments, onAddAdjustment, onUpdateReceivingCost, onDeleteInventoryItem }){
  ...
  Object.entries(editCosts).forEach(([k,c])=>{ if(ledger[k]) ledger[k].costPerLb=c; });
                    ...
                    <>
                      <tr key={i} style={{ borderBottom: isOpen?"none":"1px solid rgba(255,255,255,0.05)", background:isOpen?"rgba(61,170,106,0.06)":"transparent", transition:"background .15s" }}>
                        ...
                        <td style={{ padding:"11px 8px", textAlign:"right" }}>
                          <input type="text" inputMode="decimal" style={{ ...inp(), width:70, padding:"4px 7px", fontSize:12, textAlign:"right", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)" }}
                            value={editCosts[key]??r.costPerLb??""} placeholder="0.00"
                            onChange={e=>setEditCosts(c=>({...c,[key]:e.target.value}))}
                            onBlur={()=>{ if(editCosts[key]!==undefined && editCosts[key]!==r.costPerLb && onUpdateReceivingCost) onUpdateReceivingCost(r.origin, r.bagMarks, editCosts[key]); }}/>
                        </td>
                        ...
                        <td style={{ padding:"11px 12px", textAlign:"right" }}>
                          <div style={{ display:"flex", gap:6, justifyContent:"flex-end" }}>
                            ...
                            <button onClick={()=>{ if(window.confirm(`Delete all receiving entries for ${r.origin} / ${r.bagMarks}? This will remove the item from inventory.`)) onDeleteInventoryItem(r.origin, r.bagMarks); }}
                              style={ghost({padding:"3px 11px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>
                              🗑
                            </button>
                          </div>
                        </td>
...
- Return ONLY the JSON object, nothing else`;
  ...
  // ── Local parse (instant, no API key) ─────────────────────────────────────
  const parseLocal = () => {
    const hasFile = !!droppedFile;
    const hasText = rawText.trim().length > 0;
    const textToParse = hasFile ? droppedFile.content : hasText ? rawText : null;
    if (!textToParse) return setError("Please drop a text file or paste message text first. (Image/PDF files require AI parsing with an API key.)");
    setError(""); setParsed(null);
    try {
      const result = parseOrderText(textToParse);
      if (!result.customer && result.products.every(p => !p.productName)) {
        setError("Could not extract order details from this text. Make sure it includes customer name and product details (e.g. '50 medium roast 1lb').");
        return;
      }
      setParsed(result);
    } catch(e) {
      setError("Parse error — please check your text format and try again.");
    }
  };

  // ── Send to AI (requires API key) ──────────────────────────────────────────
  const parseWithAI = async () => {
    const hasFile = !!droppedFile;
    const hasText = rawText.trim().length > 0;
    if(!hasFile && !hasText) return setError("Please drop a file or paste message text first.");
    if(!ANTHROPIC_KEY) return setError("No API key configured. Use '⚡ Quick Parse' instead, or set REACT_APP_ANTHROPIC_KEY in your Vercel environment variables and redeploy.");
    ...
    try {
      ...
      const hasFilePDF = hasFile && droppedFile && droppedFile.type==="pdf";
      if(hasFilePDF) poHeaders["anthropic-beta"] = "pdfs-2024-09-25";

      console.log("[PO] calling Anthropic model=claude-haiku-4-5-20251001 hasFile=" + hasFile + " hasText=" + hasText);
      const poResp = await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",
        headers: poHeaders,
      ...
      console.log("[PO] response status:", poResp.status);
      if(!poResp.ok){ const t=await poResp.text(); console.error("[PO] error:", t); throw new Error("API " + poResp.status + ": " + t.slice(0,200)); }
      const data = await poResp.json();
      console.log("[PO] content:", JSON.stringify(data.content || data.error).slice(0,300));
      if(data.error) throw new Error(data.error.message||"API error");
      const rawBlock = data.content && data.content.find(function(c){return c.type==="text";});
      const rawPO = rawBlock ? rawBlock.text : "{}";
      const cleanedPO = rawPO.replace(/```[a-z]*/g,"").replace(/```/g,"").trim();
      let p = {};
      try{ p = JSON.parse(cleanedPO); }catch(_){ throw new Error("AI returned: " + rawPO.slice(0,120)); }
      if(!p.customer && !p.products) throw new Error("Could not extract order details — include customer name and product info");
      if(!p.products || !p.products.length) p.products = [{productName:"",roastProfile:"",packageSize:"",skuQty:"",totalLbs:"",requiresRoast:true,requiresGrinding:false,requiresBlending:false,requiresPackaging:true,notes:""}];
      setParsed(p);
    } catch(e){
          ...
          {confirmed ? (
            ...
            <>
              ...
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                <button onClick={parseLocal} style={mkBtn("#A855F7",{fontSize:13,padding:"10px 28px",display:"flex",alignItems:"center",gap:8})}>
                  <span>⚡</span><span>Quick Parse</span>
                </button>
                {ANTHROPIC_KEY && (
                  <button onClick={parseWithAI} disabled={parsing} style={mkBtn(parsing?"rgba(168,85,247,0.4)":"rgba(168,85,247,0.5)",{fontSize:13,padding:"10px 28px",opacity:parsing?0.7:1,display:"flex",alignItems:"center",gap:8,border:"1px solid #A855F755"})}>
                    {parsing
                      ? <><span style={{display:"inline-block",animation:"spin 1s linear infinite"}}>⏳</span><span>Parsing with AI…</span></>
                      : <><span>✨</span><span>AI Parse (images & PDFs)</span></>}
                  </button>
                )}
              </div>
              {!ANTHROPIC_KEY && <div style={{ marginTop:10, fontSize:11, color:"rgba(255,255,255,0.3)", fontStyle:"italic" }}>💡 Quick Parse works instantly on pasted text. To parse images/PDFs with AI, add your API key to Vercel env vars.</div>}
            </>
...
function ProductionOrders({ orders, onCreateOrder, onViewOrder, onDeleteOrder, customers, inventoryRows }){
  ...
  const handleDelete=(po)=>{
    if(!window.confirm(`Delete production order ${po.poNumber} for ${po.customer}?`)) return;
    onDeleteOrder(po.id);
  };

  const handleCopy=(po)=>{
    // Map old product IDs to new ones and carry over step data
    const idMap = {};
    const newProducts = po.products.map(p=>{
      const newId = Date.now()+Math.floor(Math.random()*9999);
      idMap[p.id] = newId;
      return { ...p, id:newId, batchNumber:nextBatch(), plannedBags:(p.plannedBags||[]).map(b=>({...b})) };
    });
    // Copy steps with remapped product IDs, clear completedAt so operator re-confirms
    const newSteps = {};
    Object.entries(po.steps||{}).forEach(([key,data])=>{
      const [oldProdId,stepName] = key.split("_");
      const newProdId = idMap[Number(oldProdId)] || idMap[oldProdId];
      if(newProdId && stepName){
        const {completedAt, batchNumber, ...fieldData} = data;
        newSteps[`${newProdId}_${stepName}`] = {...fieldData};
      }
    });
    const newOrder = {
      id: Date.now(),
      ...
      {showForm && (
        <div style={{ background:"rgba(168,85,247,0.06)", border:"1px solid rgba(168,85,247,0.22)", borderRadius:12, padding:22, marginBottom:24 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))", gap:"13px 18px", marginBottom:22 }}>
            ...
            <div><label style={lbl}>Customer / Account *</label>
              <input list="customer-list" type="text" style={inp()} value={hdr.customer} onChange={e=>sh("customer",e.target.value)} placeholder="Select or type customer name"/>
                  ...
                  <div style={{ display:"flex", alignItems:"flex-start", gap:14, flexWrap:"wrap" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8 }}>
                      ...
                      <div style={{ display:"flex", gap:6 }}>
                        <button onClick={()=>handleCopy(po)} style={ghost({padding:"5px 12px",fontSize:11,color:"#D4A853",borderColor:"#D4A85355"})}>📋 Copy</button>
                        <button onClick={()=>printReport(po)} style={ghost({padding:"5px 12px",fontSize:11,color:"#3DAA6A",borderColor:"#3DAA6A55"})}>🖨 Print</button>
                        <button onClick={()=>handleDelete(po)} style={ghost({padding:"5px 12px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>🗑 Delete</button>
        ...
        <POImporter
          onImported={parsed=>{
            const po={
              ...
              notes:parsed.notes||"",
              products:(parsed.products||[]).map(p=>({...p,id:Date.now()+Math.floor(Math.random()*9999),batchNumber:nextBatch()})),
...
function getStepFields(step){
  return {
    roast:[
      ...
      {name:"colorScore",label:"Color Score",type:"text"},
...
function PODetailModal({ po, inventoryRows, onClose, onUpdateStep, onDeleteStep, onToggleRequired, onChangeStatus, onUpdateOrder, onCopyOrder, customers }){
      ...
      <div style={{ maxWidth:920, margin:"0 auto", background:"#141010", border:"1px solid rgba(168,85,247,0.3)", borderRadius:14, overflow:"hidden" }}>
        ...
        <div style={{ background:"rgba(168,85,247,0.1)", borderBottom:"1px solid rgba(168,85,247,0.2)", padding:"15px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            {!editingOrder && <button onClick={()=>{onCopyOrder(po);onClose();}} style={ghost({padding:"6px 14px",fontSize:12,color:"#D4A853",borderColor:"#D4A85355"})}>📋 Copy Order</button>}
                    ...
                    return (
                      <div key={step} style={{ border:`1px solid ${done?meta.accent+"40":"rgba(255,255,255,0.08)"}`, borderRadius:9, overflow:"hidden" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:11, padding:"11px 15px", background:done?`${meta.accent}0D`:"rgba(255,255,255,0.02)", cursor:!done?"pointer":"default" }}
                          ...
                          {done && !isEditing && <button onClick={e=>{e.stopPropagation();if(window.confirm(`Delete the ${meta.label} step data for ${prod.productName}? This will clear all recorded data for this step.`)){onDeleteStep(po.id,stepKey);}}} style={ghost({padding:"3px 10px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>Delete Step</button>}
                          {!done && <button onClick={e=>{e.stopPropagation();if(window.confirm(`Remove the ${meta.label} requirement from ${prod.productName}?`)){onToggleRequired(po.id,prod.id,step,false);}}} style={ghost({padding:"3px 10px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>Remove Step</button>}
...
// ─── CUPPING LOG ──────────────────────────────────────────────────────────────
const CUPP_EMPTY_SAMPLE = {
  sampleRef:"", customer:"", origin:"", bagMarks:"", roastDate:"", roastLevel:"", roastTemp:"",
  colorResult:"pass", tasteResult:"pass", smellResult:"pass", notes:""
};
const CUPP_EMPTY = {
  date:today(), cupper:"",
  samples:[{...CUPP_EMPTY_SAMPLE, _id:1}]
};

function CuppingLog({ cuppingRecords, onSave, onUpdate, onDelete, orders, inventoryRows }){
  ...
  const sampleResult = (s) => (s.colorResult==="pass"&&s.tasteResult==="pass"&&s.smellResult==="pass")?"PASS":"FAIL";
  const overallResult = (rec) => {
    const samples = rec.samples || [];
    if(samples.length===0) return "—";
    return samples.every(s=>sampleResult(s)==="PASS") ? "PASS" : "FAIL";
  };

  // Auto-populate specs from inventory + roast data when a sample ref is selected
  const autoFillSample = (sampleRef) => {
    let origin="", bagMarks="", roastDate="", roastLevel="", roastTemp="", customer="";
    // Try to match from orders
    for(const po of (orders||[])){
      if(sampleRef===po.poNumber){
        customer = po.customer||"";
        const p=po.products[0];
        if(p) { roastLevel=p.roastProfile||""; }
        // Check every product's roast step for date
        for(const prod of po.products){
          const rd = po.steps[`${prod.id}_roast`];
          if(rd){
            roastDate = rd.roastDate||"";
            roastTemp = rd.tempEnd||rd.tempStart||"";
            if(rd.selectedBags?.length>0){
              origin = rd.selectedBags[0].origin||"";
              bagMarks = rd.selectedBags[0].bagMarks||"";
            }
            if(roastDate) break;
          }
        }
        // Fallback: use order date if no roast date found
        if(!roastDate) roastDate = po.date||"";
        break;
      }
      for(const p of po.products){
        const ref = `${po.poNumber} / ${p.batchNumber}`;
        if(sampleRef===ref || sampleRef===p.batchNumber){
          customer = po.customer||"";
          roastLevel = p.roastProfile||"";
          const roastKey = `${p.id}_roast`;
          const roastData = po.steps[roastKey];
          if(roastData){
            roastDate = roastData.roastDate||"";
            roastTemp = roastData.tempEnd||roastData.tempStart||"";
          }
          if(roastData?.selectedBags?.length>0){
            const bag = roastData.selectedBags[0];
            origin = bag.origin||"";
            bagMarks = bag.bagMarks||"";
          }
          // Fallback: use order date if no roast step date
          if(!roastDate) roastDate = po.date||"";
          break;
        }
      }
    ...
    // Try to match from inventory if still missing
    if(!origin && (inventoryRows||[]).length>0){
      const match = inventoryRows.find(r=>sampleRef.includes(r.bagMarks)||sampleRef.includes(r.origin));
      if(match){ origin=match.origin; bagMarks=match.bagMarks; }
    }
    return { origin, bagMarks, roastDate, roastLevel, roastTemp, customer };
  };

  const updateSample = (samples, _id, key, value) => {
    const updated = samples.map(s=>s._id===_id?{...s,[key]:value}:s);
    // Auto-fill when sampleRef changes
    if(key==="sampleRef" && value){
      const auto = autoFillSample(value);
      return updated.map(s=>s._id===_id?{...s,...auto,[key]:value}:s);
    }
    return updated;
  };

  const addSample = (samples) => [...samples, {...CUPP_EMPTY_SAMPLE, _id:Date.now()+Math.floor(Math.random()*9999)}];
  const removeSample = (samples, _id) => samples.filter(s=>s._id!==_id);

  const save = ()=>{
    if(!d.samples.some(s=>s.sampleRef||s.origin)) return alert("Add at least one sample with a reference or origin.");
    const rec = {...d, result:overallResult(d), id:Date.now(), createdAt:new Date().toISOString()};
    onSave(rec);
    setD({...CUPP_EMPTY, samples:[{...CUPP_EMPTY_SAMPLE, _id:Date.now()}]}); setShowForm(false);
    setFlash("saved"); setTimeout(()=>setFlash(""),2500);
  };

  const saveEdit = ()=>{
    const rec = {...editData, result:overallResult(editData)};
    onUpdate(rec); setEditId(null);
    setFlash("edited"); setTimeout(()=>setFlash(""),2500);
  };

  const handleDelete = (rec) => {
    if(!window.confirm(`Delete this cupping session?`)) return;
    onDelete(rec.id);
    setFlash("deleted"); setTimeout(()=>setFlash(""),2500);
  };

  const resultColor = r => r==="PASS"||r==="pass" ? "#3DAA6A" : r==="FAIL"||r==="fail" ? "#E8531A" : "rgba(255,255,255,0.3)";

  const PassFailToggle = ({ label, value, onChange }) => (
    <div>
      <label style={lbl}>{label}</label>
      <div style={{ display:"flex", gap:0, borderRadius:6, overflow:"hidden", border:"1px solid rgba(255,255,255,0.15)" }}>
        {["pass","fail"].map(opt=>(
          <button key={opt} onClick={()=>onChange(opt)} style={{
            flex:1, padding:"7px 12px", border:"none", cursor:"pointer", fontFamily:"inherit", fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em",
            background: value===opt ? (opt==="pass"?"rgba(61,170,106,0.25)":"rgba(232,83,26,0.25)") : "rgba(255,255,255,0.03)",
            color: value===opt ? (opt==="pass"?"#3DAA6A":"#E8531A") : "rgba(255,255,255,0.3)",
          }}>{opt==="pass"?"✓ Pass":"✕ Fail"}</button>
        ))}
      </div>
    </div>
  );

  const SampleRefSelect = ({ value, onChange }) => (
  ...
  const renderSampleForm = (sample, samples, setSamples, idx) => (
    <div key={sample._id} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(91,164,164,0.18)", borderRadius:9, padding:14, marginBottom:10 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <div style={{ fontSize:12, color:"#5BA4A4", fontWeight:700 }}>Sample {idx+1}</div>
        {samples.length>1 && <button onClick={()=>setSamples(removeSample(samples,sample._id))} style={ghost({padding:"3px 10px",fontSize:11,color:"#E8531A",borderColor:"#E8531A44"})}>✕ Remove</button>}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))", gap:"10px 16px", marginBottom:12 }}>
        <div><label style={lbl}>Sample Reference</label><SampleRefSelect value={sample.sampleRef} onChange={v=>setSamples(updateSample(samples,sample._id,"sampleRef",v))}/></div>
        <div><label style={lbl}>Customer</label><Fld type="text" value={sample.customer||""} onChange={v=>setSamples(updateSample(samples,sample._id,"customer",v))}/></div>
        <div><label style={lbl}>Origin</label><Fld type="text" value={sample.origin} onChange={v=>setSamples(updateSample(samples,sample._id,"origin",v))}/></div>
        <div><label style={lbl}>Bag Marks</label><Fld type="text" value={sample.bagMarks} onChange={v=>setSamples(updateSample(samples,sample._id,"bagMarks",v))}/></div>
        <div><label style={lbl}>Roast Date</label><Fld type="date" value={sample.roastDate} onChange={v=>setSamples(updateSample(samples,sample._id,"roastDate",v))}/></div>
        <div><label style={lbl}>Roast Level</label>
          <select style={inp()} value={sample.roastLevel||""} onChange={e=>setSamples(updateSample(samples,sample._id,"roastLevel",e.target.value))}>
            <option value="">— select —</option>
            {["Light","Medium-Light","Medium","Medium-Dark","Dark"].map(o=><option key={o}>{o}</option>)}
          </select>
        </div>
        <div><label style={lbl}>Roast Temp (°F)</label><Fld type="text" value={sample.roastTemp||""} onChange={v=>setSamples(updateSample(samples,sample._id,"roastTemp",v))}/></div>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"10px 16px", marginBottom:10 }}>
        <PassFailToggle label="Color" value={sample.colorResult||"pass"} onChange={v=>setSamples(updateSample(samples,sample._id,"colorResult",v))}/>
        <PassFailToggle label="Taste" value={sample.tasteResult||"pass"} onChange={v=>setSamples(updateSample(samples,sample._id,"tasteResult",v))}/>
        <PassFailToggle label="Smell" value={sample.smellResult||"pass"} onChange={v=>setSamples(updateSample(samples,sample._id,"smellResult",v))}/>
      </div>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
        <span style={{ fontSize:11, color:"rgba(255,255,255,0.4)" }}>Sample Result:</span>
        <span style={{ fontSize:13, fontWeight:700, color:resultColor(sampleResult(sample)) }}>{sampleResult(sample)}</span>
      </div>
      <div><label style={lbl}>Tasting Notes</label><Fld type="textarea" value={sample.notes||""} onChange={v=>setSamples(updateSample(samples,sample._id,"notes",v))} placeholder="Flavor profile, aroma, body…"/></div>
    </div>
  );

  const renderForm = (rec, setRec, onSubmit, submitLabel)=>{
    const samples = rec.samples || [{...CUPP_EMPTY_SAMPLE, _id:1}];
    const setSamples = (newSamples) => setRec("samples", typeof newSamples === "function" ? newSamples(samples) : newSamples);
    return (
      <div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:"12px 18px", marginBottom:18 }}>
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            <div style={{ fontSize:12, fontWeight:700, color:"#5BA4A4", textTransform:"uppercase", letterSpacing:"0.08em" }}>Samples ({samples.length})</div>
            <button onClick={()=>setSamples(addSample(samples))} style={ghost({fontSize:11,padding:"5px 12px",color:"#5BA4A4",borderColor:"#5BA4A466"})}>+ Add Sample</button>
          </div>
          {samples.map((s,i)=>renderSampleForm(s, samples, setSamples, i))}
        </div>

        <div style={{ padding:"10px 14px", background:"rgba(91,164,164,0.1)", borderRadius:8, marginBottom:16, display:"flex", alignItems:"center", gap:14 }}>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.5)" }}>Overall Session Result:</div>
          <div style={{ fontSize:20, fontWeight:700, color:resultColor(overallResult(rec)), letterSpacing:"0.06em" }}>{overallResult(rec)}</div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)" }}>(all samples must pass)</div>
        </div>
  ...
  return (
    <div>
      ...
      {flash==="deleted" && <div style={{ marginBottom:12, color:"#E8531A", fontSize:13 }}>✓ Session deleted.</div>}
      ...
      {cuppingRecords.length>0 && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {[...cuppingRecords].reverse().map(rec=>{
            const result = rec.result || overallResult(rec);
            const samples = rec.samples || [];
            ...
            return (
              <div key={rec.id} style={{ border:`1px solid rgba(91,164,164,0.25)`, borderRadius:10, overflow:"hidden" }}>
                ...
                <div style={{ background:"rgba(91,164,164,0.08)", padding:"13px 18px", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                      <span style={{ fontSize:15, fontWeight:700, color:"#F5EDD8" }}>{samples.length} sample{samples.length!==1?"s":""}</span>
                      {samples.slice(0,3).map((s,i)=><Badge key={i} label={s.sampleRef||s.origin||`#${i+1}`} color="#A855F7"/>)}
                      {samples.length>3 && <span style={{ fontSize:11, color:"rgba(255,255,255,0.3)" }}>+{samples.length-3} more</span>}
                    </div>
                  ...
                  <div style={{ textAlign:"center" }}>
                    <div style={{ fontSize:22, fontWeight:700, color:resultColor(result), letterSpacing:"0.06em" }}>{result}</div>
                    <div style={{ fontSize:9, color:"rgba(255,255,255,0.3)", textTransform:"uppercase", letterSpacing:"0.08em" }}>Result</div>
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
                    {!isEditing && <button onClick={()=>{ setEditId(rec.id); setEditData({...rec, samples:(rec.samples||[]).map(s=>({...s,_id:s._id||Date.now()+Math.floor(Math.random()*9999)}))}); }} style={ghost({padding:"5px 13px",fontSize:11,color:"#5BA4A4",borderColor:"#5BA4A455"})}>Edit</button>}
                    {!isEditing && <button onClick={()=>handleDelete(rec)} style={ghost({padding:"5px 13px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>Delete</button>}
                ...
                {/* Sample details */}
                {!isEditing && samples.length>0 && (
                  <div style={{ padding:"12px 18px 14px" }}>
                    {samples.map((s,i)=>(
                      <div key={i} style={{ padding:"10px 12px", background:i%2===0?"rgba(255,255,255,0.02)":"transparent", borderRadius:6, marginBottom:4 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:14, flexWrap:"wrap", marginBottom:4 }}>
                          <span style={{ fontSize:12, fontWeight:700, color:"#A855F7" }}>Sample {i+1}: {s.sampleRef||"—"}</span>
                          {s.customer && <span style={{ fontSize:11, color:"#F5EDD8" }}>👤 {s.customer}</span>}
                          {s.origin && <span style={{ fontSize:11, color:"#C8702A" }}>{s.origin}</span>}
                          {s.bagMarks && <Badge label={s.bagMarks} color="#C8702A"/>}
                          {s.roastLevel && <Badge label={s.roastLevel} color="#E8531A"/>}
                          {s.roastTemp && <span style={{ fontSize:11, color:"rgba(255,255,255,0.45)" }}>🌡 {s.roastTemp}°F</span>}
                          {s.roastDate && <span style={{ fontSize:11, color:"rgba(255,255,255,0.35)" }}>🔥 {s.roastDate}</span>}
                        </div>
                        <div style={{ display:"flex", gap:14, alignItems:"center", flexWrap:"wrap" }}>
                          {[["Color",s.colorResult],["Taste",s.tasteResult],["Smell",s.smellResult]].map(([label,val])=>(
                            <div key={label} style={{ display:"flex", alignItems:"center", gap:4 }}>
                              <span style={{ fontSize:10, color:"rgba(255,255,255,0.4)" }}>{label}:</span>
                              <span style={{ fontSize:11, fontWeight:700, color:resultColor(val||"pass"), textTransform:"uppercase" }}>{val||"pass"}</span>
                            </div>
                          ))}
                          <span style={{ fontSize:12, fontWeight:700, color:resultColor(sampleResult(s)) }}>{sampleResult(s)}</span>
                        </div>
                        {s.notes && <div style={{ marginTop:4, fontSize:11, color:"rgba(255,255,255,0.4)", fontStyle:"italic" }}>📝 {s.notes}</div>}
                      </div>
                    ))}
                  </div>
                )}
...
export default function CoffeeCRM(){
  ...
  const loadCustomers = useCallback(async()=>{
    try{ const d=await fetchCustomers(); if(d.length>0) setCustomers(d); }catch(e){ /* silently fail */ }
  },[]);

  // Derive customers from orders as fallback if customers table is empty
  const allCustomers = customers.length>0 ? customers : [...new Set(orders.map(o=>o.customer).filter(Boolean))];

  useEffect(()=>{
    Promise.all([loadOrders(),loadReceiving(),loadAdj(),loadCupping(),loadCustomers()])
      .finally(()=>setLoading(false));
    const unsub = subscribeAll(loadOrders,loadReceiving,loadAdj,loadCupping,loadCustomers);
    return unsub;
  },[loadOrders,loadReceiving,loadAdj,loadCupping,loadCustomers]);
  ...
  const handleCreateOrder = async(o)=>{
    setOrders(p=>[o,...p]);
    try{ await insertOrder(o); }catch(e){ setSyncError(e.message); }
    // Auto-save customer name
    if(o.customer && !customers.includes(o.customer)){
      setCustomers(p=>[...p,o.customer]);
      insertCustomer(o.customer);
    }
  };
  ...
  const handleDeleteStep=(poId,stepKey)=>{
    setOrders(prev=>prev.map(o=>{
      if(o.id!==poId) return o;
      ...
      const updated={...o, steps:newSteps, status};
      updateOrder(updated).catch(e=>setSyncError(e.message));
      return updated;
    }));
  };
  const handleToggleRequired=(poId,prodId,step,required)=>{
    setOrders(prev=>prev.map(o=>{
      if(o.id!==poId) return o;
      const newProducts = o.products.map(p=>{
        if(p.id!==prodId) return p;
        return {...p, [`requires${step.charAt(0).toUpperCase()+step.slice(1)}`]: required};
      });
      const newSteps = {...o.steps};
      if(!required) delete newSteps[`${prodId}_${step}`];
      const allKeys=[];
      newProducts.forEach(p=>PROCESS_STEPS.forEach(st=>{ if(p[`requires${st.charAt(0).toUpperCase()+st.slice(1)}`]) allKeys.push(`${p.id}_${st}`); }));
      const status = allKeys.length===0?"Open":allKeys.every(k=>newSteps[k])?"Closed":allKeys.some(k=>newSteps[k])?"In Progress":"Open";
      const updated={...o, products:newProducts, steps:newSteps, status};
      updateOrder(updated).catch(e=>setSyncError(e.message));
      return updated;
    }));
  };
  ...
  const handleUpdateReceivingCost = (origin, bagMarks, newCost) => {
    // Update all receiving records matching this origin/bagMarks
    setReceiving(prev=>prev.map(r=>{
      if(r.origin===origin && r.bagMarks===bagMarks){
        const updated = {...r, costPerLb:newCost};
        updateReceiving(updated).catch(e=>setSyncError(e.message));
        return updated;
      }
      return r;
    }));
  };
  const handleDeleteInventoryItem = async(origin, bagMarks) => {
    // Delete all receiving records matching this origin/bagMarks
    const toDelete = receiving.filter(r=>r.origin===origin && r.bagMarks===bagMarks);
    setReceiving(prev=>prev.filter(r=>!(r.origin===origin && r.bagMarks===bagMarks)));
    for(const r of toDelete){
      try{ await deleteReceiving(r.id); }catch(e){ setSyncError(e.message); }
    }
  };
  ...
  const handleDeleteOrder=async(id)=>{
    ...
    try{ await deleteOrder(id); }catch(e){ setSyncError(e.message); }
  };
  const handleDeleteReceiving=async(id)=>{
    setReceiving(p=>p.filter(r=>r.id!==id));
    try{ await deleteReceiving(id); }catch(e){ setSyncError(e.message); }
  };
  const handleDeleteCupping=async(id)=>{
    setCupping(p=>p.filter(r=>r.id!==id));
    try{ await deleteCupping(id); }catch(e){ setSyncError(e.message); }
  };

  // ── Excel Export ────────────────────────────────────────────────────────────
  const exportToExcel = (tabName) => {
    const script = document.createElement("script");
    script.src = "https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js";
    script.onload = () => {
      const XLSX = window.XLSX;
      let data = [];
      let filename = `${tabName}_report_${today()}.xlsx`;

      if(tabName==="production"){
        data = orders.map(po=>({
          "PO #": po.poNumber, Customer: po.customer, Date: po.date, "Target Date": po.targetDate,
          Priority: po.priority, Status: po.status,
          Products: po.products.map(p=>`${p.productName} (${p.roastProfile||""} ${p.packageSize||""}) x${p.skuQty||""}`).join("; "),
          Notes: po.notes||""
        }));
      } else if(tabName==="receiving"){
        data = receiving.map(r=>({
          Date: r.date, Supplier: r.supplier, Origin: r.origin, "Bag Marks": r.bagMarks,
          "Qty Bags": r.qtyBags, "lbs/Bag": r.weightPerBag, "Cost/lb": r.costPerLb,
          "DO #": r.deliveryOrder, Notes: r.notes||""
        }));
      } else if(tabName==="inventory"){
        data = inventoryRows.map(r=>{
          const bal=r.bagsIn-r.bagsOut;
          return {
            Origin: r.origin, "Bag Marks": r.bagMarks, "Bags In": r.bagsIn, "Bags Out": r.bagsOut,
            Balance: bal, "lbs/Bag": r.weightPerBag||"", "Cost/lb": r.costPerLb||"",
            "Total lbs": bal*(Number(r.weightPerBag)||0), Status: bal<=0?"Empty":bal<=5?"Low Stock":"In Stock"
          };
        });
      } else if(tabName==="cupping"){
        data = [];
        cuppingRecords.forEach(r=>{
          const samples = r.samples || [];
          if(samples.length===0){
            // Legacy record without samples array
            data.push({
              Date: r.date, Cupper: r.cupper, Customer: "", "Sample Ref": r.sampleRef||"",
              Origin: r.origin||"", "Bag Marks": r.bagMarks||"", "Roast Date": r.roastDate||"",
              "Roast Level": r.roastLevel||"", "Roast Temp": "",
              Color: r.colorResult||"", Taste: r.tasteResult||"", Smell: r.smellResult||"",
              "Sample Result": "", "Session Result": r.result||"", Notes: r.notes||""
            });
          } else {
            samples.forEach((s,i)=>{
              data.push({
                Date: r.date, Cupper: r.cupper, Customer: s.customer||"",
                "Sample #": i+1, "Sample Ref": s.sampleRef||"",
                Origin: s.origin||"", "Bag Marks": s.bagMarks||"", "Roast Date": s.roastDate||"",
                "Roast Level": s.roastLevel||"", "Roast Temp (°F)": s.roastTemp||"",
                Color: s.colorResult||"", Taste: s.tasteResult||"", Smell: s.smellResult||"",
                "Sample Result": (s.colorResult==="pass"&&s.tasteResult==="pass"&&s.smellResult==="pass")?"PASS":"FAIL",
                "Session Result": r.result||"", Notes: s.notes||""
              });
            });
          }
        });
        if(data.length===0) data=[{Note:"No cupping records found"}];
      } else {
        // roast, grinding, blending, packaging — extract step data from orders
        data = [];
        orders.forEach(po=>{
          po.products.forEach(prod=>{
            const stepKey = `${prod.id}_${tabName}`;
            const stepData = po.steps[stepKey];
            if(stepData){
              data.push({
                "PO #": po.poNumber, Customer: po.customer, Batch: prod.batchNumber,
                Product: prod.productName, "Roast Profile": prod.roastProfile||"",
                ...Object.fromEntries(Object.entries(stepData).filter(([k])=>!["selectedBags","completedAt","batchNumber"].includes(k)).map(([k,v])=>[k.replace(/([A-Z])/g," $1").trim(), Array.isArray(v)?JSON.stringify(v):v])),
                "Completed At": stepData.completedAt?new Date(stepData.completedAt).toLocaleDateString():""
              });
            }
          });
        });
        if(data.length===0) data=[{Note:`No ${tabName} records found`}];
      }
    ...
    <div style={{ minHeight:"100vh", background:"#0E0A07", fontFamily:"'Georgia','Times New Roman',serif", color:"#F5EDD8", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, padding:"20px 22px", maxWidth:1200, width:"100%", boxSizing:"border-box" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16, paddingBottom:14, borderBottom:`1px solid ${meta.accent}30` }}>
          ...
          <button onClick={()=>exportToExcel(active)} style={ghost({padding:"6px 14px",fontSize:11,color:"#3DAA6A",borderColor:"#3DAA6A55",display:"flex",alignItems:"center",gap:6})}>
            📊 Export Excel
          </button>
        </div>
      ...
      {viewPO && <PODetailModal po={viewPO} inventoryRows={inventoryRows} onClose={()=>setViewingPO(null)} onUpdateStep={handleUpdateStep} onDeleteStep={handleDeleteStep} onToggleRequired={handleToggleRequired} onChangeStatus={handleChangeStatus} onUpdateOrder={handleUpdateOrder} onCopyOrder={handleCopyOrder} customers={allCustomers}/>}
    </div>
  );
}
