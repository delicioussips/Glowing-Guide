...
function ProductionOrders({ orders, onCreateOrder, onViewOrder, onDeleteOrder, customers, inventoryRows }){
  ...
  const handleCopy=(po)=>{
    // Build ID mapping: old product ID → new product ID
    const idMap = {};
    ...
    // Copy step data using known product IDs and step names
    const newSteps = {};
    po.products.forEach(oldProd=>{
      const newProdId = idMap[String(oldProd.id)];
      PROCESS_STEPS.forEach(step=>{
        const oldKey = `${oldProd.id}_${step}`;
        const oldData = po.steps[oldKey];
        if(oldData){
          const {completedAt, batchNumber, ...fieldData} = oldData;
          newSteps[`${newProdId}_${step}`] = {...fieldData};
        }
      });
    });
    const newOrder = {
