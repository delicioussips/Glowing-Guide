...
- Return ONLY the JSON object, nothing else`;
  ...
  const parseWithAI = async () => {
    ...
    try {
      ...
      const poBody = { model:"claude-3-5-haiku-20241022", max_tokens:2000, system:AI_SYSTEM, messages:[{ role:"user", content:messageContent }] };
      if(hasFilePDF) poBody._beta = "pdfs-2024-09-25";

      console.log("[PO] calling proxy hasFile=" + hasFile + " hasText=" + hasText);
      const poResp = await fetch("/.netlify/functions/claude",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
