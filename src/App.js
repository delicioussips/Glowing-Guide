...
function ProductionOrders({ orders, onCreateOrder, onViewOrder, onDeleteOrder, customers, inventoryRows }){
  ...
  const handleCopy=(po)=>{
    // Create new products with new IDs
    const newProducts = [];
    const newSteps = {};
    po.products.forEach((oldProd, idx)=>{
      const newId = Date.now() + idx * 7 + Math.floor(Math.random()*99);
      const newProd = { ...oldProd, id:newId, batchNumber:nextBatch(), plannedBags:(oldProd.plannedBags||[]).map(b=>({...b})) };
      newProducts.push(newProd);
      // Copy ALL step data for this product, remapped to new ID
      PROCESS_STEPS.forEach(step=>{
        const oldKey = String(oldProd.id) + "_" + step;
        const oldData = (po.steps||{})[oldKey];
        if(oldData && typeof oldData === "object"){
          newSteps[newId + "_" + step] = JSON.parse(JSON.stringify(oldData));
        }
      });
    });
    const newOrder = {
      ...
      steps: newSteps,
      status: Object.keys(newSteps).length > 0 ? "In Progress" : "Open",
