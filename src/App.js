...
const ANTHROPIC_KEY     = process.env.REACT_APP_ANTHROPIC_KEY || "";import { useState, useEffect, useRef, useCallback } from "react";
import { createClient } from '@supabase/supabase-js';
...
function CuppingLog({ cuppingRecords, onSave, onUpdate, onDelete, orders, inventoryRows }){
  ...
  const addSample = (samples) => [...samples, {...CUPP_EMPTY_SAMPLE, _id:Date.now()+Math.random()}];
  const removeSample = (samples, _id) => samples.filter(s=>s._id!==_id);
            ...
            return (
              <div key={rec.id} style={{ border:`1px solid rgba(91,164,164,0.25)`, borderRadius:10, overflow:"hidden" }}>
                ...
                <div style={{ background:"rgba(91,164,164,0.08)", padding:"13px 18px", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    {!isEditing && <button onClick={()=>{ setEditId(rec.id); setEditData({...rec, samples:(rec.samples||[]).map(s=>({...s,_id:s._id||Date.now()+Math.random()}))}); }} style={ghost({padding:"5px 13px",fontSize:11,color:"#5BA4A4",borderColor:"#5BA4A455"})}>Edit</button>}
                    {!isEditing && <button onClick={()=>handleDelete(rec)} style={ghost({padding:"5px 13px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>Delete</button>}
                    {isEditing  && <button onClick={()=>setEditId(null)} style={ghost({padding:"5px 13px",fontSize:11,color:"#E8531A",borderColor:"#E8531A55"})}>Cancel</button>}
                  </div>
                </div>
