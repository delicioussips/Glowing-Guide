...
const CUPP_EMPTY_SAMPLE = {
  sampleRef:"", origin:"", bagMarks:"", roastDate:"", roastLevel:"", roastTemp:"",
...
function CuppingLog({ cuppingRecords, onSave, onUpdate, onDelete, orders, inventoryRows }){
  ...
  const autoFillSample = (sampleRef) => {
    let origin="", bagMarks="", roastDate="", roastLevel="", roastTemp="";
    // Try to match from orders
    for(const po of (orders||[])){
      if(sampleRef===po.poNumber){
        // PO level — take first product info
      ...
      for(const p of po.products){
        ...
        if(sampleRef===ref || sampleRef===p.batchNumber){
          ...
          // Check for bag marks from step data
    ...
    return { origin, bagMarks, roastDate, roastLevel, roastTemp };
  };
  ...
  const exportToExcel = (tabName) => {
    ...
    script.onload = () => {
      ...
      if(tabName==="production"){
        ...
        data = cuppingRecords.map(r=>({
          Date: r.date, Cupper: r.cupper, "Sample Refs": (r.sampleRefs||[r.sampleRef]).filter(Boolean).join(", "),
          Origin: r.origin, "Bag Marks": r.bagMarks, "Roast Level": r.roastLevel,
