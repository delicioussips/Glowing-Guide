  ...
  const handleDOUpload = async (file)=>{
    ...
    reader.onload = async(e)=>{
      ...
      try {
        ...
        // PDFs need the extended thinking / document beta
        ...
        const resp = await fetch("https://api.anthropic.com/v1/messages",{
          ...
          body: JSON.stringify({
            ...
            max_tokens:2000,
            system:`You extract delivery order line items from documents. Return ONLY a valid JSON array — no markdown, no explanation, no backticks.
Each element in the array represents one coffee lot/line item:
[{"date":"YYYY-MM-DD or empty","supplier":"shipper name or empty","origin":"coffee origin country or region","bagMarks":"bag marks or lot ID","qtyBags":"integer string","weightPerBag":"lbs per bag as number string","deliveryOrder":"DO number","notes":"any notes"}]
If multiple lots are listed, return multiple objects. Share date/supplier/DO across all items.`,
            messages:[{ role:"user", content:[contentBlock,{ type:"text", text:"Extract all line items from this delivery order. Return only the JSON array." }]}]
          })
        });
        ...
        let parsed = [];
        try { parsed = JSON.parse(text.replace(/```json|```/g,"").trim()); } catch(e){ throw new Error("Could not parse AI response: "+text.slice(0,80)); }
        if(!Array.isArray(parsed)) parsed = parsed ? [parsed] : [];
        if(parsed.length===0) throw new Error("No line items found in document. Try a clearer scan.");
        ...
        setDoError(err.message||"Could not parse document.");
      }
      setUploading(false);
    };
  ...
  const parseWithAI = async () => {
    ...
    try {
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({ model:"claude-opus-4-6", max_tokens:2000, system:AI_SYSTEM, messages:[{ role:"user", content:messageContent }]})
      });
      const data = await resp.json();
      const text = data.content?.find(c=>c.type==="text")?.text||"{}";
      let p = {};
