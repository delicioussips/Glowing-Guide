  ...
  const autoFillSample = (sampleRef) => {
    ...
    for(const po of (orders||[])){
      if(sampleRef===po.poNumber){
        ...
        // Check roast step for first product
        if(p){
          const roastData = po.steps[`${p.id}_roast`];
          if(roastData){
            roastDate = roastData.roastDate||"";
            roastTemp = roastData.tempEnd||roastData.tempStart||"";
          }
          if(roastData?.selectedBags?.length>0){
            origin = roastData.selectedBags[0].origin||"";
            bagMarks = roastData.selectedBags[0].bagMarks||"";
          }
        }
        break;
      }
